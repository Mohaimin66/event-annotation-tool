...
function handleTokenClick(idx, shiftKey) {
    if (shiftKey && selectedTriggerIndices.length > 0) {
        // Range selection
        const lastSelected = selectedTriggerIndices[selectedTriggerIndices.length - 1];
...
function handleSearch(query) {
    ...
    const matches = eventTypes.filter(et =>
...
function setupEventListeners() {
    ...
    modalSearchInput.addEventListener('input', (e) => renderModalResults(e.target.value));
